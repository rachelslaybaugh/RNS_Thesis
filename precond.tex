...
See the results for varying the weight with 1 relaxation per level and 1 V-cycles in Table~\ref{table:FxdSrcTstVacWeight}. The results for varying with number of relaxations per grid and the number of V-cycles with the weight fixed at one can be seen in Table~\ref{table:FxdSrcTstVacRV}. When the preconditioning parameters were really cranked up nothing changed, see Table~\ref{table:FxdSrcTstVacOther}.
%
\begin{table}[!h]
...
Both the weight variation and relaxations + V-cycle study can be seen together in Figure~\ref{fig:FxdSrcVac}.
%
\begin{figure}[!ht]
    \begin{center}
...
See the results for varying the weight with 1 relaxation per level and 1 V-cycles in Table~\ref{table:FxdSrcTstReflWeight}. The results for varying with number of relaxations per grid and the number of V-cycles with the weight fixed at one can be seen in Table~\ref{table:FxdSrcTstReflRV}. 
%
\begin{table}[!h]
...
Both the weight variation and relaxations + V-cycle study can be seen together in Figure~\ref{fig:FxdSrcRefl}.
%
\begin{figure}[!ht]
    \begin{center}
...
This problem also very briefly investigated the option of using a different angle set within the preconditioner. The overall problem was solved with $S_{8}$ and the preconditioner only used $S_{2}$. For the w1 r2 v2 case the time was reduced from 7.07 $\times 10^{2}$ seconds to 1.57 $\times 10^{2}$ seconds. 
%
\begin{table}[!h]
...
\hline
Solver & GS iters & total Krylov & time (s)\\[0.5ex]
\hline
...
Results are in Table~\ref{table:2D c5g7}. All $k$ values were within the uncertainty of the benchmark and so are not reported. These were run on the small orthanc cluster at Oak Ridge with 16 cores; 4 x-blocks and 4 y-blocks, 1 z-block and 1 set. The total and upscatering tolerances were 1 $\times 10^{-3}$, with a k tolerance of 1 $\times 10^{-5}$. In the table ``Krylov'' is the total number of Krylov iterations and ``Eigenvalue'' is the total number of Eigenvalue iterations. 
%
\begin{table}[!h]
...
\hline
Weight & Relaxations & V-cycles & Krylov & Eigenvalue & Time (s) \\[0.5ex]
\hline
...
\hline
1.4 & 2 & 2 & 438   & 31 & 1.77 $\times 10^{4}$ \\
1    & 3 & 3 & 253   & 31 & 2.28 $\times 10^{4}$ \\
\hline 
\end{tabular} 
\end{center}
\label{table:2D c5g7}
\end{table}
 %
 This information can also be seen in Figure~\ref{fig:2Dc5g7PI}.
 %
\begin{figure}[!ht]
    \begin{center}
...
%
\begin{table}[!h]
\caption{3D C5G7 Benchmark Preconditioning With Power Iteration, Parameter Scoping}
\begin{center}
\begin{tabular}{c c c c c c}
\hline
Weight & Relaxations & V-cycles & Krylov & Eigenvalue & Time (s) \\[0.5ex]
...
\hline 
\end{tabular}\\
$^{*}$tol and upscatter tol = 1 $\times 10^{-5}$, $k$ tol = 1 $\times 10^{-3}$; this $k$ value was low by 0.015.
\end{center}
\label{table:3D c5g7}
\end{table}
...
\begin{tabular}{c c c c c}
\hline
Weight & Relaxations & V-cycles & Krylov & Eigenvalue \\[0.5ex]
\hline
0    & 0 & 0 & 39 & 6 \\
1    & 1 & 1 & 27 & 6 \\
1.2 & 1 & 1 & 31 & 6 \\
...
This is the small RQI unit test with reflecting boundary conditions, the same that was discussed in Chapter~\ref{sec:Chp3}. Many of these tests did not pass with the preconditioner. The flux was not tested against a very tight tolerance, either $1 \times 10^{-2}$ or $1 \times 10^{-3}$ unless otherwise noted. The results are show in Table~\ref{table:RQIUnitTestRefl}. Note that here the max number of Krylov iterations was set to 100.


* parameter variation with rqi reflecting + shifted operator, 4 groups (RQI\_UnitTest\_refl)
This is the small RQI unit test with reflecting boundary conditions. This test also used the shifted version of the operator in the preconditioner. Many of these tests did not pass with the preconditioner. The flux was not tested against a very tight tolerance, either $1 \times 10^{-2}$ or $1 \times 10^{-3}$ unless otherwise noted. The results are show in Table~\ref{table:RQIUnitTestReflShifted}. Note that here the max number of Krylov iterations was set to 100.
%
\begin{table}[!h]
...
\begin{tabular}{c c c c c c}
\hline
Weight & Relaxations & V-cycles & $k$ & Krylov & Eigenvalue \\[0.5ex]
\hline
0    & 0 & 0 & 2 & 35 & 5 \\
1    & 1 & 1 & test failed \\
1.4 & 1 & 1 & test failed \\
...
Multisets: shows that preconditioner actually works great with multisets and we probably chose the right implementation.
  multisets with fixed source, 27 groups (FeC\_vac)

  multisets with power iteration, 27 groups (im\_pi)
 
  multisets with power iteration, 44 groups (Full\_PWR) ***Not done at all***
 
  multisets with rqi, 44 groups (Full\_PWR) ***Not done at all***
