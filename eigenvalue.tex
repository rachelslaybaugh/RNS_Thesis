...
To see how this will work, subtract $\rho \ve{TF}$ from both sides of $(\ve{I} - \ve{TS})\phi = \lambda \ve{TF}\phi$, where $\rho$ is the Rayleigh quotient and $\lambda = \frac{1}{k}$. This gives the following shifted system:
%
\begin{align}
  [\ve{I} - \ve{T}(\ve{S} + \rho\ve{F})]\phi &=( \lambda - \rho) \ve{T}\ve{F} \phi \:. \\
  \text{ Now define } \ve{\tilde{S}} &\equiv \ve{S} + \rho\ve{F} \text{ to write} \nonumber \\
   [\ve{I} - \ve{T}\ve{\tilde{S}}]\phi &=( \lambda - \rho) \ve{T}\ve{F} \phi \:.
...
The energy-parallelized RQI method is shown in Algorithm \ref{algo:RQI+MGkrylov}. Here $\ve{A} = [\ve{I} - \ve{TS}]$ and a block of energy groups corresponding to an energy set is denoted by superscript ${\tilde{g}}$. The moments being computed by a given energy set are indicated by $\phi^{\tilde{g}}$.
%
\begin{algorithm}[!h]
  ...
  \begin{list}{}{\hspace{2.5em}}
     ...
     \begin{enumerate}
       \item subtracting $\rho_{old} [\ve{TF}]^{\tilde{g}}$ from $\ve{A}^{\tilde{g}}$ and 
       \item making the right hand side $\mu [\ve{TF}]^{\tilde{g}} \phi^{\tilde{g}}$
    \end{enumerate}
    \item Use MG Krylov solver to solve $[\ve{I} - \ve{T}\ve{\tilde{S}}]^{\tilde{g}}\phi^{\tilde{g}} = \mu [\ve{TF}]^{\tilde{g}} \phi^{\tilde{g}}$ for an updated $\phi^{\tilde{g}}$
...
\end{algorithm}
%
Denovo's implementation of RQI uses the generalized form of the Rayleigh quotient: $\frac{\phi^{T} \ve{A} \phi}{\phi^{T} \ve{TF} \phi}$. The way this is computed using energy sets is shown in Algorithm \ref{algo:calcRQ}. 
%
\begin{algorithm}[!h]
  \begin{list}{}{\hspace{2.5em}}
    \item denominator $= \phi_{old}^{\tilde{g}, T} \ve{TF}^{\tilde{g}} \phi_{old}^{\tilde{g}}$
...
It is likely that the eigenvector does not converge because RQI creates poorly conditioned systems. Krylov methods do not handle ill-conditioned systems very well and may converge extremely slowly. Further, in such a scenario GMRES may no longer be backwards stable. If GMRES is not backward stable, then there is no guarantee that RQI will converge. The observed behavior strongly suggests that the multigroup Krylov method should be preconditioned so that the eigenvector will converge. If that happens it is likely that RQI will then converge as well. 
