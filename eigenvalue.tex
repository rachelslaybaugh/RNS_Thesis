...
\end{equation}
\begin{alignat}{2}
  \text{let } \ve{T} = \ve{DL}^{-1} \nonumber \:, \text{ then multiply both sides by }f^{T}
\end{alignat}
\begin{align}
  f^{T}(\ve{I} - \ve{TMS})\phi &= \frac{f^{T}}{k} \ve{TM}\chi f^{T} \phi \:,
    \label{eq:OperatorEvalForm} \\
  f^{T}\phi^{k+1} &=  \frac{f^{T}}{k}(\ve{I} - \ve{TMS})^{-1} \ve{TM} \chi f^{T}\phi^{k} \:.
...
It is worth noting that there is no evidence that, given a sufficient amount of computing time, RQI would converge to the wrong answer. Because GMRES is monotonically convergent the eigenvector should converge given a sufficient amount of computing time and resources. There is certainly not data to support the claim that RQI will converge to the proper eigenvalue in all cases, but it is heartening to notice there is also not data indicating that it will not. 
